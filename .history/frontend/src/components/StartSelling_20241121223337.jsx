...
const StartSelling = () => {
  ...
  const handleFileChange = (e, fieldName) => {
    const file = e.target.files[0];
    if (file) {
      // Update storeInfo with the file object
      setStoreInfo((prev) => ({
        ...prev,
        [fieldName]: file,
      }));
  
      // Optional: Generate preview URLs if needed
  ...
  const handleClick = async (e) => {
    ...
    formData.append("business_logo", storeInfo.businessLogo); // File input
    formData.append("business_background", storeInfo.businessBackground); // File input
