...
const TopProductsChart = () => {
  ...
  // Total units sold (used for percentage calculation)
  const totalUnits = topProducts.reduce((sum, product) => sum + product.total_sold, 0);
  ...
  const options = {
    plugins: {
      datalabels: {
        formatter: (value) => `${((value / totalUnits) * 100).toFixed(1)}%`, // Calculate percentage
        color: '#fff', // Label text color
        font: {
          size: 14, // Label font size
        },
        anchor: 'center', // Position label inside the slice
        align: 'center',
      },
      legend: {
        display: true,
