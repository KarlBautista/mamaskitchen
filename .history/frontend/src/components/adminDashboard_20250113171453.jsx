    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.data.success) {
          Swal.fire('Shipped!', 'The order has been marked as shipped.', 'success');
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.order_id}>
                  ...
                  <td className={`status ${order.status.toLowerCase()}`}>{order.status || "Pending"}</td>
                  <td>
                    ...
                    {["Shipped", "Received", "Cancelled"].includes(order.status) && (
                      <button onClick={() => handleRemoveOrder(order.order_id)}>
                        Remove
                      </button>
                    )}
                  </td>
