...
const StartSelling = () => {
  ...
  const handleClick = async (e) => {
    ...
    const payload = {
      ...
      business_logo: businessLogo?.name || "", // Send file name or empty string
      business_background: businessBackground?.name || "", // Send file name or empty string
    };
  
    try {
      const response = await axios.post("http://localhost:8800/stores_account", payload);
