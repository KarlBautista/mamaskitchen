...
function Header() {
    ...
    // Detect changes to the `user` context
    useEffect(() => {
        if (user) {
            setIsDropdownOpen(false); // Ensure dropdown is closed on login
        }
    }, [user]);
    ...
    const toggleDropdown = () => {
        setActiveLink('user'); // Set active link to 'user' when dropdown is opened
    ...
    return (
        <div className={`header-container ${isScrolled ? 'scrolled' : ''} ${showHeader ? 'visible' : 'hidden'}`}>
            ...
            <div className="header-last">
                <div
                    className={`bayong-container ${activeLink === 'cart' ? 'active' : ''}`}
                    onClick={() => handleLinkClick('cart')}
                >
                    <i className="fa-solid fa-cart-shopping bayong"></i>
                </div>
                <div
                    className={`user-dropdown ${activeLink === 'user' ? 'active' : ''}`}
                >
