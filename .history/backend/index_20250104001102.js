...
app.post('/add_food', async (req, res) => {
  ...
  const query = `
    INSERT INTO foods (name, price, category, image_url)
    VALUES (?, ?, ?, ?);
  `;

  try {
    const result = await db.query(query, [name, price, category, image_url]);
    res.json({ success: true, newDish: { name, price, category, image_url, id: result.insertId } });
  } catch (err) {
...
// Update a food item
app.put('/update_food/:id', async (req, res) => {
  const { id } = req.params;
  const { name, price, category, image_url } = req.body;

  try {
    const result = await db.query(
      'UPDATE foods SET name = ?, price = ?, category = ?, image_url = ? WHERE id = ?',
      [name, price, category, image_url, id]
    );
    res.json({ success: true, updatedDish: { id, name, price, category, image_url } });
  } catch (err) {
