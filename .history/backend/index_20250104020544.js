...
app.post('/checkout', async (req, res) => {
  const { user_id, total_price, order_date, items } = req.body;

  if (!user_id || !total_price || !items || items.length === 0) {
    return res.status(400).json({ success: false, message: 'Invalid order data' });
  }

  const query = 'INSERT INTO orders (user_id, total_price, order_date) VALUES (?, ?, ?)';
  const values = [user_id, total_price, order_date];

  try {
    const [orderResult] = await db.query(query, values);

    // Get the last inserted order ID
    const orderId = orderResult.insertId;

    // Insert items into order_items table
    const orderItemsQuery = 'INSERT INTO order_items (order_id, food_name, price, quantity) VALUES ?';
    const orderItemsValues = items.map(item => [orderId, item.food_name, item.price, item.quantity]);
