...
const [orders] = await db.query(ordersQuery, [user_id]);

const enrichedOrders = orders.map(order => {
  const revenue = order.quantity * order.price; // Revenue per item
  return {
    ...order,
...
res.json({ success: true, orders: enrichedOrders });
  ...
  try {
    ...
    if (orders.length > 0) {
      ...
      // Add revenue (10% of the price) for each order item
      const ordersWithRevenue = groupedOrders.map(order => {
        order.total_revenue = order.items.reduce((total, item) => {
          const revenue = item.price * item.quantity * 0.1; // 10% revenue
          return total + revenue;
        }, 0);
        return order;
      });

      res.json({ success: true, orders: ordersWithRevenue });
    } else {
