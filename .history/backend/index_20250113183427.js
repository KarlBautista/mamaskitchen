...
app.put('/update_food/:id', upload.single('image'), async (req, res) => {
  ...
  try {
    // Fetch the existing data for the food item
    const [existingFood] = await db.query('SELECT * FROM foods WHERE id = ?', [id]);
    ...
    const updatedName = name || existingFood[0].name;
    const updatedPrice = price || existingFood[0].price;
    const updatedCategory = category || existingFood[0].category;
    const updatedImageUrl = image ? `/uploads/${image.filename}` : existingFood[0].image_url;

    // Update the food item with the new or existing data
    const query = `
      UPDATE foods 
      SET name = ?, price = ?, category = ?, image_url = ? 
      WHERE id = ?
    `;
    const values = [updatedName, updatedPrice, updatedCategory, updatedImageUrl, id];
